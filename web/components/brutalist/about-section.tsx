...
function UptimeCounter() {
  ...
  useEffect(() => {
    const base = 86400 * 2 + Math.floor(Math.random() * 50000);
    setSeconds(base);
    ...
    <section id="problem" className="w-full px-6 py-20 lg:px-12 scroll-mt-24">
      ...
      <div className="flex flex-col lg:flex-row gap-0 border-2 border-foreground">
        <motion.div
          ...
          className="relative w-full lg:w-1/2 min-h-[300px] lg:min-h-[500px] border-b-2 lg:border-b-0 lg:border-r-2 border-foreground overflow-hidden bg-foreground text-background p-6 lg:p-8 flex flex-col"
        >
          <div className="flex items-center justify-between mb-8">
            <span className="text-[10px] tracking-[0.2em] uppercase text-background/60 font-mono">
              RENDER: repo_as_db.obj
            </span>
          ...
          <pre className="flex-1 font-mono text-[11px] sm:text-xs leading-relaxed text-background/85 overflow-auto">
{`repo/
├── docs/
│   ├── users/
│   │   ├── ada.json
...
# every write = a commit
# every delete = a real tree rebuild
# history = git log on the path`}
          </pre>

          <div className="mt-6 flex items-center justify-between pt-4 border-t border-background/20">
        ...
        >
          ...
          <div className="flex-1 flex flex-col justify-between px-5 py-6 lg:py-8">
            <div className="flex flex-col gap-6">
              ...
              >
                Managed DBs bill you forever.
                <br />
                <span className="text-[#ea580c]">You already pay for GitHub.</span>
              </motion.h2>
              ...
              >
                <p className="text-xs lg:text-sm font-mono text-muted-foreground leading-relaxed">
                  Prototypes and side projects do not need another cloud
                  database account. git3 stores collections as JSON in a private
                  repo you own — with Mongo-style add / find / set / remove.
                </p>
                <p className="text-xs lg:text-sm font-mono text-muted-foreground leading-relaxed">
                  Open Studio locally with one command. Paste a token. Browse
                  documents, edit JSON, inspect path history. Ship the SDK in
